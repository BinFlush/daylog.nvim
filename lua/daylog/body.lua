...
-- :DaylogOrder's reorder and its change-warning sort through this one rule, so a
...
local function sorted_items(items)
  ...
  -- Sort by effective UTC time so :DaylogOrder agrees with the effective
...
-- that differs (as { minutes, text }), so :DaylogOrder can warn that it set
