...
-- Locator for a blotter's generated summary region -- the banner-delimited blast.
--
-- A blotter is a body (the `--- blots ... ---` header, its timestamped blots and
...
-- next blotter / EOF: nothing inside it is authored, so a refresh regenerates the
...
-- the body above the boundary and, absolutely, the blots.
--
-- So the locator's whole job is to find the TOP BOUNDARY -- where the body ends and
-- the summary begins -- and return [boundary .. next-blotter/EOF). The boundary is
-- the summary banner:
...
--     `--- summary q=15 d=dec EDITED ---`, guarded to stay below the last blot and
...
-- summary after the body. The window is anchored past the last blot, a hard
-- guarantee that a blot can never be drawn into the zone and rewritten away.
...
-- The tail window: from just after the blotter's last timestamped blot to the next
-- blotter header / EOF. Returns tail_start, stop_row (1-based; stop_row exclusive,
-- = first row past the tail). Anchoring past the blots guarantees they stay out of
-- the located zone.
...
-- The canonical banner string this blotter would render, the target the
-- mangled-banner search aligns against.
local function canonical_banner(blotter_block)
  return syntax.summary_header(blotter_block.quantize_minutes, blotter_block.duration_format)
end
...
-- content by shape. Scan the tail from just past the last blot; the zone starts at
...
-- summary -- the blotter's summary is contiguous from its body, so a stray
...
-- Locate `blotter_block`'s generated summary region, returning { start_row, end_row }
-- (1-based, end_row exclusive) covering the whole summary zone -- from the banner
-- (the body/summary boundary) to the next blotter / EOF -- or nil when no summary is
...
function M.find(analysis, blotter_block, _expected_lines)
  local tail_start, stop_row = tail_bounds(analysis, blotter_block)
  if not tail_start then
    return nil
  end

  local start = find_banner(analysis, tail_start, stop_row, canonical_banner(blotter_block))
...
-- The blotter's tail bounds (tail_start, stop_row): just past the last blot, to the
-- next blotter / EOF. Exposed so the create path can blast to the same zone end.
function M.tail_bounds(analysis, blotter_block)
  return tail_bounds(analysis, blotter_block)
end
