return function(t)
  ...
  -- Surgical, exact-output regeneration scenarios (the whole document, byte for byte).

  t.test("regen: a dropped dash on the second header recovers the whole document", function()
    t.eq(
      regen({
        "--- blots ---",
        "09:00 a",
  ...
  t.test("regen: only the corrupted middle blotter of three is repaired", function()
    t.eq(
      regen({
        "--- blots ---",
        "08:00 a",
        ...
        "",
        "",
        "--- blts ---", -- corrupted keyword on the middle blotter
        "12:00 b",
        "13:00 done",
        "",
        "",
  ...
  t.test("regen: an obliterated header inherits the previous blotter's metadata", function()
    -- The obliterated header carried q=45; with nothing readable, the synthesized header
    -- (and its summary) inherit the previous blotter's #proj q=20.
    t.eq(
      regen({
        "--- blots #proj q=20 ---",
        "09:00 a",
        ...
        "",
        "",
        "scratch reminder text", -- the header, obliterated to prose
        "13:00 b",
        "14:00 done",
        "",
        "",
  ...
  t.test(
    "regen: a corrupted header keeps every option, including #tag @loc utc and d=hm",
    function()
      t.eq(
        regen({
